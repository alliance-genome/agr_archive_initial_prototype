...
class ZFIN(MOD):
    ...
    def load_go(self):
        ...
        for row in query.rows():
            list.append({"gene_id": row["primaryIdentifier"], "go_id": row["goAnnotation.ontologyTerm.identifier"],
    ...
    def load_diseases(self):
        ...
        for row in query.rows():
            if row["phenotypeLink.identifier"] is not None:
                list.append(
                    {"gene_id": row["genes.primaryIdentifier"], "omim_id": "OMIM:" + row["phenotypeLink.identifier"],
                     "species": ZFIN.species})
        return list

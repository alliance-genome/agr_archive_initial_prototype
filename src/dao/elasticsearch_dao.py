...
class ElasticSearchDAO(object):
    ...
    def __init__(self, app):
        ...
        if app.config['ES_AWS']:
            self.es = Elasticsearch(app.config['ES_HOST'], timeout=30, retry_on_timeout=False, use_ssl=True, verify_certs=True)
        else:
            self.es = Elasticsearch(app.config['ES_HOST'], timeout=30, retry_on_timeout=False)
